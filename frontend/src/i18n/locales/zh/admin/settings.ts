export default {
    settings: {
      ...
      transcription: {
        title: '语音识别',
        description: '管理网页聊天的云端语音转文字模型。这里的模型只负责把录音写入输入框，不会改变左侧选择的 GPT 回答模型。',
        loading: '正在加载语音识别设置',
        ...
        groups: '承担语音识别的资源组',
        groupsHint: '可选择启用中的 OpenAI 标准资源组；已经失效的旧选择会保留显示，方便你移除或紧急关闭功能。',
        groupMeta: '资源组 #{id} · {accounts} 个启用账号',
        groupUnavailable: '此资源组已停用，请取消选择或先恢复资源组',
        missingGroup: '已不存在的资源组 #{id}',
        removeMissingGroup: '取消勾选即可清理这条旧配置',
        groupsLoadFailed: '资源组列表暂时加载失败',
        noGroups: '没有可选择的标准资源组',
        noGroupsHint: '请先创建一个 OpenAI 标准资源组，并把至少一个启用“音频转写”能力的 API Key 账号加入该组。',
        failoverTitle: '账号失效时自动切换',
        failoverHint: '模型不会绑定到单个 API Key。同一资源组内所有可调度的语音账号都会参与选择；过期、停用或限流账号会被自动跳过。',
        manageAccounts: '管理语音上游账号',
        availableInGroups: '{count} 个资源组当前可用',
        modelUnavailable: '当前没有可用上游',
        availabilityPending: '保存时校验可用性',
        deploymentDefault: '沿用部署默认值',
        catalogUnavailableTitle: '暂时无法检测上游模型',
        catalogUnavailableHint: '调度器当前不可用。你仍可关闭功能，但启用或切换模型前应先恢复上游账号。',
        runtimeUnavailableTitle: '当前服务器节点未准备好语音识别',
        runtimeUnavailableHint: '该节点缺少 ffprobe 或请求体安全上限不足。你仍可关闭功能；恢复节点配置后再启用。',
        unavailableTitle: '所选模型当前不可用',
        unavailableHint: '请检查资源组、API Key 状态、“音频转写”端点能力及账号模型映射，然后重新加载。',
        dailyQuotaTitle: '单用户每日语音额度',
        dailyQuotaHint: '控制每位用户每天可送往云端转写的录音总时长。达到上限后只暂停语音输入，文字聊天不受影响；调高会增加云端转写费用。',
        dailyQuotaLabel: '每日额度',
        dailyQuotaUnit: '秒 / 用户 / 天',
        dailyQuotaEquivalent: '约 {minutes} 分钟；额度在北京时间每天 08:00 重置。',
        dailyQuotaInvalid: '请输入 {min} 到 {max} 之间的整数秒数。',
        safetyLimits: '部署安全上限',
        safetyLimitsHint: '以下上传、并发与超时限制由服务器配置锁定，后台不能放大；修改上方额度和模型不需要重启服务。',
        maxUpload: '单次上传',
        maxDuration: '录音时长',
        globalConcurrency: '全局并发',
        userConcurrency: '单用户并发',
        userRPM: '单用户 RPM',
        dailySeconds: '每日语音秒数',
        requestTimeout: '上游超时',
        ...
        statusReady: '运行正常',
        statusDisabled: '已关闭',
        statusUnavailable: '等待可用上游',
        statusPending: '有待校验的更改',
        unsaved: '有尚未保存的语音识别设置',
        saved: '设置已同步到数据库',
        reload: '重新检测',
        discardConfirm: '重新检测会丢弃尚未保存的语音识别更改，确定继续吗？',
        save: '保存并立即生效',
        saveSuccess: '语音识别设置已更新，无需重启服务',
        saveFailed: '保存语音识别设置失败',
      },
      features: {
        channelMonitor: {
          title: '渠道监控',
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于落雪API用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于落雪API用户登录',
      ...
      customMenu: {
        ...
        authMode: '外部页面认证',
        authModeNone: '不认证',
        authModeExchangeCode: '一次性交换码',
        authModeHint: '交换码模式只发送短时、单次可用的授权码，不会暴露本站会话 Token。',
        add: '添加菜单项',
        remove: '删除',
        moveUp: '上移',
        moveDown: '下移',
      ...
      payment: {
        ...
        balanceRechargeMultiplierHint: '用户每支付 1 CNY 可获得多少雪花额度',
        balanceRechargePreview: '预览：1 CNY = {credit} 雪花额度',
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到落雪API订单中，上游 type 会作为易支付 type 参数提交。',
      ...
      balanceNotify: {
        ...
        enabled: '启用余额不足提醒',
        threshold: '默认提醒阈值（雪花额度）',
        thresholdHint: '用户未自定义时使用此雪花额度值',
