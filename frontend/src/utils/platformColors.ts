...
export type Platform = 'anthropic' | 'openai' | 'antigravity' | 'gemini' | 'grok' | 'kimi' | 'zhipu' | 'deepseek'
...
export function resolveGroupDisplayPlatform(
  ...
  // Some legacy Moonshot/Kimi groups were created as generic OpenAI
  // compatible groups before Kimi became a first-class platform. Keep their
  // routing platform unchanged, but show the provider brand in badges/icons.
  if (
...
const BADGE_LIGHT: Record<Platform, string> = {
  ...
  kimi: 'bg-pink-500/10 text-pink-600 dark:text-pink-300',
...
const ACCENT_BAR: Record<Platform, string> = {
  ...
  zhipu: 'bg-gradient-to-r from-indigo-400 to-indigo-600',
...
function isPlatform(p: string): p is Platform {
  return p === 'anthropic' || p === 'openai' || p === 'antigravity' || p === 'gemini' || p === 'grok' || p === 'kimi' || p === 'zhipu' || p === 'deepseek'
}
