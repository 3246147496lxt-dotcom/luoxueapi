...
describe('user dashboard workspace surface contract', () => {
  ...
  it('keeps one period selector and the approved orange-to-blue chart hierarchy', () => {
    ...
    expect(charts).toContain("'--workspace-work-chart-cost'")
    expect(charts).toContain("'--workspace-work-chart-primary'")
    expect(charts).toContain("'--workspace-work-chart-secondary'")
    expect(charts).not.toMatch(/#(?:7c3aed|6d28d9|5b21b6|8b5cf6|a78bfa|c4b5fd)/i)
  ...
  it('keeps the lower insight cards on real model and channel hooks', () => {
    ...
    expect(insights).toContain('selectedMonitorId.value = preferredMonitor?.id ?? null')
    expect(insights).toContain(':value="monitor.id"')
  ...
  it('keeps approved typography and Workspace values in the canonical token source', () => {
    ...
    expect(workspaceTokens).toContain('--workspace-light-work-accent: #2563eb;')
    expect(workspaceTokens).toContain('--workspace-dark-work-accent: #60a5fa;')
    expect(workspaceTokens).toContain('--workspace-light-work-chart-cost: #ea580c;')
    expect(workspaceTokens).toContain('--workspace-dark-work-chart-cost: #fb923c;')
    expect(workspaceTokens).toContain('--workspace-work-accent: var(--workspace-dark-work-accent);')
    ...
    expect(workspaceTokens).toContain('--workspace-light-dashboard-text-subtle: #64748b;')
