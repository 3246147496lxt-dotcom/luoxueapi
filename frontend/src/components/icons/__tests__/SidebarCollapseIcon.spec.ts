...
describe('SidebarCollapseIcon', () => {
  it('uses the target filled sidebar glyph in both expanded and collapsed states', async () => {
    ...
    expect(svg.get('path').attributes('d')).toBe(TARGET_SIDEBAR_ICON_PATH)

    await wrapper.setProps({ collapsed: true })

    expect(wrapper.get('path').attributes('d')).toBe(TARGET_SIDEBAR_ICON_PATH)
  })
})
