  ...
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      <div class="flex items-center gap-3 rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="flex-1"><p class="font-medium text-gray-900">{{ user.email }}</p><p class="flex items-center gap-1 text-sm text-gray-500"><span>{{ t('admin.users.currentBalance') }}:</span><CreditAmount :value="formatBalance(user.balance)" icon-size="xs" /></p></div>
      </div>
      <div>
        <label class="input-label">{{ operation === 'add' ? t('admin.users.depositAmount') : t('admin.users.withdrawAmount') }}</label>
        <div class="relative flex gap-2">
          <div class="relative flex-1"><span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-gray-500" aria-hidden="true">$</span><input v-model.number="form.amount" type="number" step="any" min="0" required class="input pl-10" /></div>
      ...
      <div v-if="form.amount > 0" class="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-950"><div class="flex items-center justify-between text-sm"><span class="text-gray-700 dark:text-gray-300">{{ t('admin.users.newBalance') }}:</span><CreditAmount class="font-bold text-gray-900 dark:text-gray-100" :value="formatBalance(calculateNewBalance())" icon-size="xs" /></div></div>
    </form>
    <template #footer>
