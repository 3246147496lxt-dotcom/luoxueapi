      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="hasQuotaLimit" @click="$emit('reset-quota', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-primary-600 hover:bg-gray-100 dark:text-primary-400 dark:hover:bg-dark-700">
              <Icon name="refresh" size="sm" />
              {{ t('admin.accounts.resetQuota') }}
            </button>
            <div class="my-1 border-t border-gray-100 lg:hidden dark:border-dark-700"></div>
            <button
              @click="$emit('delete', account); $emit('close')"
              class="flex min-h-11 w-full items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 lg:hidden dark:text-red-400 dark:hover:bg-red-900/20"
            >
