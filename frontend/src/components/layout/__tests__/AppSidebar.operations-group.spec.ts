...
describe('AppSidebar grouped admin navigation', () => {
  ...
  it('keeps the three overview icon shapes fixed in the shared navigation definition', () => {
    const icons = {
      ...
      accountAssistant: Symbol('account-assistant'),
  ...
  it('keeps the configured model catalog in Workbench and support links as auxiliary links', () => {
    ...
    const modelCenterIcon = modelCenterLink.get('.sidebar-svg-icon svg')
    expect(modelCenterIcon.attributes('viewBox')).toBe('0 0 1024 1024')
    expect(modelCenterIcon.findAll('path')).toHaveLength(3)
    expect(modelCenterIcon.find('path').attributes('fill')).toBe('currentColor')
