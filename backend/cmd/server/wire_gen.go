...
func initializeApplication(buildInfo handler.BuildInfo, cfg *config.Config, entClient *ent.Client, db *sql.DB, redisClient *redis.Client) (*Application, error) {
	...
	deepSeekBalanceService := service.ProvideDeepSeekBalanceService(accountRepository, proxyRepository, httpUpstream, cfg)
	cnProviderQuotaService := service.ProvideCNProviderQuotaService(accountRepository, proxyRepository, httpUpstream, cfg)
	accountHandler := admin.ProvideAccountHandler(accountAdminUseCases, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, deepSeekBalanceService, cnProviderQuotaService)
	accountHandler.SetAccountHealthSettingsRepository(settingRepository)
	accountHealthService := service.NewAccountHealthServiceFrom(accountAdminUseCases, accountTestService)
	accountHealthService.SetSettingsRepository(settingRepository)
	accountHealthRunner := service.NewAccountHealthRunner(accountHealthService)
