...
func TestSubscriptionAnchoredMonthlyQuotaMigrationFailsClosedOnUnreconstructableReceipts(t *testing.T) {
	...
	require.Contains(t, sql, "user_subscriptions_usage_amounts_check")
	...
	require.Contains(t, sql, "ul.actual_cost = 'NaN'::numeric")
	require.Contains(t, sql, "g.weekly_limit_usd = 'NaN'::numeric")
	require.Contains(t, sql, "g.monthly_limit_usd = 'NaN'::numeric")
	require.Contains(t, sql, "subscription_amount <> 'Infinity'::numeric")
	require.Contains(t, sql, "invalid_subscription_time_count")
	require.Contains(t, sql, "authoritative_weekly_ledger_mismatch_count")
...
func TestSubscriptionAnchoredMonthlyQuotaMigrationPreservesPlanConfiguration(t *testing.T) {
	...
	require.NotContains(t, upperSQL, "SET MONTHLY_LIMIT_USD =")
	require.NotContains(t, upperSQL, "SET WEEKLY_LIMIT_USD =")
}
