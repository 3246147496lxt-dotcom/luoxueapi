-- Migration: 248_channel_monitor_deepseek_provider
-- Allow DeepSeek as a channel-monitor provider.  DeepSeek uses the
-- OpenAI-compatible Chat Completions protocol at its public API endpoint.
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('deepseek' IN monitor_constraint_def) = 0 THEN
        ...
        ALTER TABLE channel_monitors
            ...
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok', 'deepseek'));
    END IF;
