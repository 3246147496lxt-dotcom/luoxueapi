...
type ProxyLatencyInfo struct {
	...
	// QualityItems keeps the per-target result needed by the ops health view.
	// It is an internal cache payload; public response models deliberately expose
	// only target/status/latency and never the raw error message or proxy URL.
	QualityItems []ProxyQualityCheckItem `json:"quality_items,omitempty"`
