...
// 请求头覆写（header override）：对 Anthropic / OpenAI / DeepSeek 平台的
// api_key 账号以及 Grok 平台的 api_key / oauth 账号生效。
...
//   - session-id/session_id/x-claude-code-session-id/x-grok-conv-id 等：逐请求会话隔离头，
//     固定值会造成会话串扰。
...
// Anthropic / OpenAI / DeepSeek 仅开放 api_key 账号；Grok 额外开放 oauth 账号——
...
func (a *Account) IsHeaderOverrideEligible() bool {
	...
	switch a.Platform {
	case PlatformAnthropic, PlatformOpenAI, PlatformZhipu, PlatformDeepseek:
