...
func defaultModelsListCandidateIDs(platform string) []string {
	...
	case PlatformDeepseek:
		...
		// GetGroupModelsListCandidates below.
		return []string{"deepseek-v4-pro", "deepseek-v4-flash", "deepseek-v4-flash-vision-exp"}
	case PlatformZhipu:
		// Keep the stable GLM catalog visible before an account has a
		// synchronized model mapping. Account-specific aliases are appended
		// below.
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	profitControlEnabled, profitMinMargin, profitSafetyBuffer := NormalizeProfitControlConfig(platform, input.ProfitControlEnabled, input.ProfitMinMargin, input.ProfitSafetyBuffer)
	...
	if group.RequireOAuthOnly && (group.Platform == PlatformOpenAI || group.Platform == PlatformAntigravity || group.Platform == PlatformAnthropic || group.Platform == PlatformGemini || group.Platform == PlatformGrok || IsCNProvider(group.Platform)) && len(accountIDsToCopy) > 0 {
