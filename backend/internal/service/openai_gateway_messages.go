...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	// First-class CN providers expose an Anthropic-compatible facade when the
	// account explicitly selects api_protocol=anthropic. Route those accounts
	// natively; otherwise an Anthropic-shaped request would be sent to the
	// provider's OpenAI Chat endpoint and lose protocol semantics.
	if account != nil && account.IsCNProvider() &&
		(account.IsAnthropicProtocol() || (account.IsDeepseek() && account.IsAdaptiveAPIProtocol())) {
		return s.forwardAnthropicViaNativeAnthropicEndpoint(ctx, c, account, body, defaultMappedModel)
	}
	// CN providers without an Anthropic protocol (including GLM's default Chat
	// mode) use the direct Anthropic→Chat bridge. An explicitly configured native
	// Responses account can still use the normal Anthropic→Responses conversion
	// path below.
	if account != nil && account.IsCNProvider() && account.GetAPIProtocol() == APIProtocolChatCompletions {
	...
	if account.Platform != PlatformGrok {
		// Preserve the explicit Codex session header across the compat bridge. A
		// rewritten body may carry a different prompt_cache_key; it must not
		// silently replace session-id/session_id affinity or collapse an explicit
		// conversation_id into that unrelated key.
		sessionSeed := explicitOpenAIRequestSessionID(c, nil)
		if sessionSeed == "" {
			sessionSeed = strings.TrimSpace(promptCacheKey)
		}
		if sessionSeed != "" {
			isolatedSessionID := generateSessionUUID(isolateOpenAISessionID(apiKeyID, sessionSeed))
		...
		if upstreamReq.Header.Get("conversation_id") != "" {
			...
			if conversationSeed != "" {
				isolatedConversationID := generateSessionUUID(isolateOpenAISessionID(apiKeyID, conversationSeed))
	...
	// 使 handler 通过专用 Cyber producer 按上游真实 token 记录用量与计费，
	// 不 failover、不重复走正常 RecordUsage。
...
func (s *OpenAIGatewayService) handleAnthropicStreamingResponse(
	...
	missingTerminalErr := func() (*OpenAIForwardResult, error) {
		...
		if !clientOutputStarted {
			return result, s.newOpenAIStreamFailoverError(c, account, false, requestID, nil, message, resp.Header)
		}
