...
func TestRecordCyberPolicyEvent_RespectsContentModerationScope(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "included scope preserves disabled automatic ban",
			config:    `{"all_groups":false,"group_ids":[7],"model_filter":{"type":"include","models":["gpt-5"]},"auto_ban_enabled":false,"ban_threshold":1}`,
			groupID:   &groupID,
			model:     "gpt-5",
			wantCalls: []bool{false},
			wantLogs:  1,
		...
		{
			name:     "included scope preserves cyber ban count exclusion",
			config:   `{"all_groups":false,"group_ids":[7],"model_filter":{"type":"include","models":["gpt-5"]},"cyber_policy_exclude_from_ban_count":true,"ban_threshold":1}`,
			groupID:  &groupID,
			model:    "gpt-5",
			wantLogs: 1,
		},
