...
func (s *AccountService) Create(ctx context.Context, req CreateAccountRequest) (*Account, error) {
	...
	if account.Type == AccountTypeAPIKey && len(req.GroupIDs) > 0 {
		for _, gid := range req.GroupIDs {
			...
			if g.RequireOAuthOnly && (g.Platform == PlatformOpenAI || g.Platform == PlatformAntigravity || g.Platform == PlatformAnthropic || g.Platform == PlatformGemini || g.Platform == PlatformGrok || IsCNProvider(g.Platform)) {
...
func (s *AccountService) TestCredentials(ctx context.Context, id int64) error {
	...
	case PlatformDeepseek:
		...
		return nil
	case PlatformZhipu:
		// Zhipu/GLM credentials are API-key based; the forwarding path performs
		// the authoritative upstream validation.
		return nil
	default:
