...
const (
	...
	// providerDeepSeekPath DeepSeek public API Chat Completions 路径。
	// DeepSeek's root endpoint is /chat/completions (without a synthetic /v1).
	providerDeepSeekPath = "/chat/completions"
	// providerZhipuPath Zhipu GLM pay-as-you-go Chat Completions path. Monitor
	// endpoints are origins, so include the provider's data-plane prefix here.
