...
type RedeemCodeRepository interface {
	...
	// ListByUserPaginated returns paginated redeem-code history for a specific user.
	// An empty codeType returns balance/concurrency history and excludes unrelated
	// redeem-code types; pass an explicit type to query that type directly.
