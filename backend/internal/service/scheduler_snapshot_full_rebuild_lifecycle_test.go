...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Len(t, cache.tokens(), 2*len(schedulerCanonicalBuckets(groupID)), "full rebuild and the following group event must each run fresh authority")
	_, reopenHeld := cache.lifecycleMutationLeaseStates()
	require.Len(t, reopenHeld, 2*len(schedulerCanonicalBuckets(groupID)))
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	require.Equal(t, 3*expectedGroupLifecycleAccountCallCount(), accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	require.Equal(t, 2*expectedGroupLifecycleAccountCallCount()+1, accounts.callCount())
	// The stale registry entry is retained as an ordinary historical bucket
	// after the active group's canonical buckets are reopened, so it incurs one
	// additional group-scoped account query.
	require.Equal(t, expectedGroupLifecycleAccountCallCount()+1, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	require.Equal(t, 2*expectedGroupLifecycleAccountCallCount()+3, accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, expectedGroupLifecycleAccountCallCount(), accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Equal(t, expectedGroupLifecycleAccountCallCount(), accounts.callCount())
	require.Empty(t, cache.tokens())
}
...
func TestSchedulerFullRebuildActiveTombstoneLazyRecoveryDiscardsPartialCaptureTasks(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.False(t, held)
		require.Equal(t, len(schedulerCanonicalBuckets(groupID)), reopenCount)
		require.Equal(t, len(schedulerCanonicalBuckets(0))+6+1, capturesAtFirstDB)
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Zero(t, fallbackCalls)
	require.Empty(t, freshCalls)
	require.Equal(t, len(schedulerCanonicalBuckets(0))+3, cache.captureAttemptCount())
	require.Equal(t, expectedGroupLifecycleAccountCallCount()+3, accounts.callCount())
	require.Equal(t, expectedGroupLifecycleAccountCallCount()+3, accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, expectedGroupLifecycleAccountCallCount()+len(schedulerSnapshotPlatforms())+2, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, 2*(expectedGroupLifecycleAccountCallCount()+len(schedulerSnapshotPlatforms())+2), accounts.callCount())
