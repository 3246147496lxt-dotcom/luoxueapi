...
func TestUpstreamResponseModelObserverSupportsAnthropicAndGeminiShapes(t *testing.T) {
	...
	t.Run("gemini wrapper disagreement keeps outer and records conflict", func(t *testing.T) {
		observer := &upstreamResponseModelObserver{}
		observer.ObserveGemini([]byte(`{"modelVersion":"gemini-outer","response":{"modelVersion":"gemini-inner"}}`))
		require.Equal(t, "gemini-outer", observer.Model())
		require.True(t, observer.Conflict())
	})

	t.Run("gemini wrapper case-only agreement keeps outer without conflict", func(t *testing.T) {
		observer := &upstreamResponseModelObserver{}
		observer.ObserveGemini([]byte(`{"modelVersion":"GEMINI-SAME","response":{"modelVersion":"gemini-same"}}`))
		require.Equal(t, "GEMINI-SAME", observer.Model())
...
func TestUpstreamModelMismatchTreatsGrokBuildRuntimeIDsAsAliases(t *testing.T) {
	tests := []struct {
		...
		{name: "grok 4.6", sentModel: "grok-4.6", responseModel: "grok-4.6-build"},
		{name: "grok 4.6 latest", sentModel: "grok-4.6-latest", responseModel: "grok-4.6-build"},
		{name: "grok 4.5 latest", sentModel: "grok-4.5-latest", responseModel: "grok-4.5-build"},
		{name: "grok 4.5 canonical", sentModel: "grok-4.5", responseModel: "GROK-4.5-BUILD"},
	}
...
func TestUpstreamModelMismatchDoesNotCollapseDifferentModels(t *testing.T) {
	tests := []struct {
		...
		{name: "different grok versions", sentModel: "grok-4.5", responseModel: "grok-4.6-build"},
		{name: "unrelated build suffix", sentModel: "gpt-5.5", responseModel: "gpt-5.5-build"},
		{name: "different grok runtime", sentModel: "grok-build-0.1", responseModel: "grok-4.5-build"},
	}
...
func TestObserveAntigravityGeminiSSELineReadsWrapperModelWithoutUnwrap(t *testing.T) {
	tests := []struct {
		...
		{name: "top-level sibling", payload: `{"modelVersion":"gemini-3-pro","response":{"candidates":[]}}`, want: "gemini-3-pro"},
		{name: "single wrapper", payload: `{"response":{"modelVersion":"gemini-3-pro","candidates":[]}}`, want: "gemini-3-pro"},
		{name: "nested response after one wrapper", payload: `{"response":{"response":{"modelVersion":"gemini-3-pro","candidates":[]}}}`, want: "gemini-3-pro"},
		{name: "outer declaration takes precedence", payload: `{"modelVersion":"gemini-outer","response":{"modelVersion":"gemini-inner","candidates":[]}}`, want: "gemini-outer", wantConflict: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.wantConflict, observedUpstreamResponseModelConflict(c))
		})
	}
}
...
func TestUpstreamResponseModelObserverRejectsMalformedJSONWithModelField(t *testing.T) {
	...
	t.Run("gemini wrapper", func(t *testing.T) {
		observer := &upstreamResponseModelObserver{}
		observer.ObserveGemini([]byte(`{"modelVersion":"gemini-outer","response":{"modelVersion":"gemini-inner"}`))
		require.Empty(t, observer.Model())
		require.False(t, observer.Conflict())
...
func TestUpstreamResponseModelObserverBoundsUntrustedModelNameByRune(t *testing.T) {
	observer := &upstreamResponseModelObserver{}
	...
	require.Len(t, observer.Model(), upstreamResponseModelMaxLength*len("模"))
}

func TestUpstreamResponseModelObserverTerminalConflictIsSticky(t *testing.T) {
	observer := &upstreamResponseModelObserver{}
	observer.Observe("early-a", false)
	observer.Observe("terminal-b", true)
	observer.Observe("terminal-b", true)

	require.Equal(t, "terminal-b", observer.Model())
	require.True(t, observer.Conflict())
}
