...
func (s *OpenAIGatewayService) forwardChatCompletionsViaNativeAnthropic(
	...
	startTime := time.Now()
	// Set the concrete endpoint up front so handler error/failover logging does
	// not fall back to the platform's default Responses path.
	...
	if err := validateCNAnthropicCrossChatRequest(&ccReq, cnAnthropicProviderLabel(account.Platform)); err != nil {
...
func (s *OpenAIGatewayService) handleCCBufferedFromNativeAnthropic(
	...
	processPayload := func(eventName, payload string) {
		...
		// it.  The shared state machine expects the JSON `type` field, so restore
		// it before observing/parsing instead of silently dropping that event.
	...
	for {
		...
		if readErr != nil {
			...
			writeChatCompletionsError(c, http.StatusBadGateway, "server_error", "Upstream stream read failed")
	...
	if !sawTerminalEvent {
		writeChatCompletionsError(c, http.StatusBadGateway, "server_error", "Upstream stream ended before message_stop")
...
func (s *OpenAIGatewayService) handleCCStreamingFromNativeAnthropic(
	...
	// Finalize both state machines only after a valid terminal event.  If the
	// client disconnected, usage was still drained above but no writes are
	// attempted on the closed writer.
	if !clientDisconnected {
