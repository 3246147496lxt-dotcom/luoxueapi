...
func (o *upstreamResponseModelObserver) ObserveGemini(payload []byte) {
	// Gemini OAuth/Antigravity responses can wrap the actual response while
	// also declaring modelVersion on the wrapper. Collect every declaration
	// from inner to outer so the trusted outer declaration remains the selected
	// terminal value while a disagreement is still recorded as a conflict.
	...
	if len(models) == 0 || !gjson.ValidBytes(payload) {
		return
	}
