...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	if s.schedulerSnapshot != nil && s.accountRepo != nil {
		...
		if requestedModel != "" &&
			!latest.IsOpenAIPassthroughEnabled() &&
			!latest.IsModelSupported(requestedModel) {
			return 0, nil, "", nil
		}
	...
	if vetoed, _ := openAIProfitControlVetoReason(ctx, account); vetoed {
		// Profit eligibility is transient; preserve the response binding so the
		// chain can resume if the account rate later recovers.
		return 0, nil, "", nil
	}
