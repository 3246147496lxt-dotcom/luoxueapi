...
// availableBalanceDeductor is intentionally narrower than UserRepository so
// normal usage billing can retain its overdraft semantics while refunds clamp
// the deduction to the balance that is actually available at commit time.
...
func (s *PaymentService) queryAndFinalizeRefund(ctx context.Context, oid int64, forceOverride bool) (*RefundResult, error) {
	...
	resp, err := queryProvider.QueryRefund(ctx, payment.RefundQueryRequest{
		...
		Amount: formatGatewayRefundAmount(
...
// finalizePendingRefundSuccess claims a pending refund and applies the
// balance/subscription deduction in the same transaction as the final order
// status and success audit. This makes retries/concurrent admin clicks
// idempotent and rolls back the deduction if any later write fails.
...
func (s *PaymentService) markRefundOkTx(ctx context.Context, client *dbent.Client, p *RefundPlan) (*RefundResult, error) {
	...
	now := time.Now()
	_, err := client.PaymentOrder.UpdateOneID(p.OrderID).
		SetStatus(fs).
