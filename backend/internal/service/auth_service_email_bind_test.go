...
func TestAuthServiceBindEmailIdentity_SameUserConcurrentTargetsKeepSingleEmailIdentity(t *testing.T) {
	cache := &emailBindCacheStub{
		data: &service.VerificationCodeData{
			Code:      "123456",
			CreatedAt: time.Now().UTC(),
	...
	require.NoError(t, err)
	user := createEmailBindTestUser(t, client, "same-user-before@example.com", "same-user", hashedPassword)
	...
	targets := []string{"same-user-first@example.com", "same-user-second@example.com"}
	start := make(chan struct{})
...
func (s *emailBindUserRepoStub) UpdateEmailWithAliasGuard(_ context.Context, userID int64, email, passwordHash string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	for id, stored := range s.usersByID {
		if id != userID && service.NormalizeEmailForAliasDedup(stored.Email) == identity {
			return service.ErrEmailExists
		}
	}
	delete(s.usersByEmail, existing.Email)
	cloned := cloneEmailBindUser(existing)
	cloned.Email = email
	cloned.PasswordHash = passwordHash
	s.usersByID[userID] = cloned
