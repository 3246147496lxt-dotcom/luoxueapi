...
func TestOpenAIGatewayMessagesDispatchGateAllowsGrokGroups(t *testing.T) {
	...
	t.Run("deepseek_group_without_dispatch_flag_reaches_gateway_dependencies", func(t *testing.T) {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", strings.NewReader(`{"model":"deepseek-chat","messages":[{"role":"user","content":"hi"}]}`))
		groupID := int64(4103)
		c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{
			ID:      5103,
			GroupID: &groupID,
...
func TestOpenAIResponsesWebSocket_ContentModerationBlocksFirstFrame(t *testing.T) {
	...
	moderationSvc.Start()
	t.Cleanup(func() { _ = moderationSvc.Stop(context.Background()) })
