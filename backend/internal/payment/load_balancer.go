...
// decryptConfig parses an AES-256-GCM provider config while accepting plaintext
// JSON records written during the temporary plaintext compatibility period.
func (lb *DefaultLoadBalancer) decryptConfig(stored string) (map[string]string, error) {
	...
	if len(lb.encryptionKey) != AES256KeySize {
		return nil, fmt.Errorf("decrypt payment provider config: TOTP_ENCRYPTION_KEY is not configured")
	}
