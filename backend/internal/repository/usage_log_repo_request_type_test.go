...
func TestPrepareUsageLogInsert_ArgCountMatchesTypes(t *testing.T) {
	...
	upstreamResponseModel := "gpt-5.4-2026-08-01"
	...
	require.Equal(t, sql.NullString{String: upstreamResponseModel, Valid: true}, prepared.args[7])
	require.Equal(t, sql.NullBool{Bool: upstreamModelMismatch, Valid: true}, prepared.args[8])
}
...
func TestUsageLogRepositoryGetUserSpendingRanking(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"user_id", "email", "username", "main_model", "actual_cost", "requests", "tokens", "total_actual_cost", "total_requests", "total_tokens"}).
		AddRow(int64(2), "beta@example.com", "Beta", "claude-sonnet-4", 12.5, int64(9), int64(900), 40.0, int64(30), int64(2600)).
		AddRow(int64(1), "alpha@example.com", "Alpha", "gpt-5", 12.5, int64(8), int64(800), 40.0, int64(30), int64(2600)).
		AddRow(int64(3), "gamma@example.com", "Gamma", "gemini-2.5-pro", 4.25, int64(5), int64(300), 40.0, int64(30), int64(2600))

	rankingQueryPattern := `(?s)WITH user_spend AS \(.*ranked_users AS \(.*ORDER BY user_spend\.actual_cost DESC, user_spend\.tokens DESC, user_spend\.user_id ASC.*LIMIT \$3.*model_usage AS \(.*COALESCE\(NULLIF\(TRIM\(u\.requested_model\), ''\), NULLIF\(TRIM\(u\.model\), ''\), ''\).*INNER JOIN ranked_users.*ORDER BY tokens DESC, requests DESC, model ASC.*ORDER BY ranked_users\.actual_cost DESC, ranked_users\.tokens DESC, ranked_users\.user_id ASC`
	mock.ExpectQuery(rankingQueryPattern).
	...
	require.Equal(t, &usagestats.UserSpendingRankingResponse{
		Ranking: []usagestats.UserSpendingRankingItem{
			{UserID: 2, Email: "beta@example.com", Username: "Beta", MainModel: "claude-sonnet-4", ActualCost: 12.5, Requests: 9, Tokens: 900},
			{UserID: 1, Email: "alpha@example.com", Username: "Alpha", MainModel: "gpt-5", ActualCost: 12.5, Requests: 8, Tokens: 800},
			{UserID: 3, Email: "gamma@example.com", Username: "Gamma", MainModel: "gemini-2.5-pro", ActualCost: 4.25, Requests: 5, Tokens: 300},
		},
...
func TestUpstreamModelMismatchConditionUsesExplicitBooleanPredicates(t *testing.T) {
	require.Equal(t, "upstream_model_mismatch IS TRUE", upstreamModelMismatchCondition("upstream_model_mismatch", true))
	require.Equal(t, "usage_logs.upstream_model_mismatch IS FALSE", upstreamModelMismatchCondition("usage_logs.upstream_model_mismatch", false))
}
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	...
	t.Run("service_tier_is_scanned", func(t *testing.T) {
		...
		log, err := scanUsageLog(usageLogScannerStub{values: []any{
			...
			sql.NullString{},
			sql.NullString{Valid: true, String: "gpt-5.4-2026-08-01"},
		...
		require.Equal(t, "gpt-5.4-2026-08-01", *log.UpstreamResponseModel)
