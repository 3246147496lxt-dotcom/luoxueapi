...
const (
	...
	userFrontendIndexFilePath  = "index.html"
	adminFrontendBasePath      = "/admin"
	adminFrontendIndexFilePath = "admin/index.html"
...
func NewFrontendServer(settingsProvider PublicSettingsProvider) (*FrontendServer, error) {
	...
	baseHTML, err := fs.ReadFile(distFS, userFrontendIndexFilePath)
	if err != nil {
		return nil, fmt.Errorf("read embedded user frontend entry: %w", err)
	}
	adminBaseHTML, err := fs.ReadFile(distFS, adminFrontendIndexFilePath)
	if err != nil {
		return nil, fmt.Errorf("read embedded admin frontend entry: %w", err)
	}

	cache := NewHTMLCache()
	cache.SetBaseHTML(baseHTML)
	adminCache := NewHTMLCache()
	adminCache.SetBaseHTML(adminBaseHTML)

	return &FrontendServer{
		distFS:        distFS,
		fileServer:    http.FileServer(http.FS(distFS)),
		baseHTML:      baseHTML,
		adminBaseHTML: adminBaseHTML,
		cache:         cache,
...
func (s *FrontendServer) Middleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Resolve each SPA entry before handing directories to http.FileServer.
		// In particular, "admin" is a real build directory and would otherwise
		// redirect /admin instead of serving the injected admin document.
		if isFrontendEntryRequest(path, cleanPath) || !s.fileExists(cleanPath) {
			s.serveIndexHTML(c)
			return
		}
...
func isAdminFrontendPath(requestPath string) bool {
	path := strings.TrimSpace(requestPath)
	return path == adminFrontendBasePath || strings.HasPrefix(path, adminFrontendBasePath+"/")
}

func frontendIndexFilePath(requestPath string) string {
	if isAdminFrontendPath(requestPath) {
		return adminFrontendIndexFilePath
	}
	return userFrontendIndexFilePath
}

func isFrontendEntryRequest(requestPath, cleanPath string) bool {
	if cleanPath == frontendIndexFilePath(requestPath) {
		return true
	}
	return requestPath == adminFrontendBasePath || requestPath == adminFrontendBasePath+"/"
}
...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	baseHTML, cache := s.frontendDocument(c.Request.URL.Path)
	if len(baseHTML) == 0 || cache == nil {
		c.String(http.StatusInternalServerError, "Frontend entry not available")
		c.Abort()
		return
	}
	...
	// Check cache first
	cached := cache.GetForKey(cacheKey)
	if cached != nil {
		// The CSP nonce changes on every response, so a conditional 304 would
		// pair the cached HTML's old nonce with the new response policy. Always
		// return a fresh body while reusing the server-side rendered template.
	...
	if err != nil {
		...
		fallback := injectRouteMetadata(baseHTML, nil, c.Request.URL.Path)
	...
	rendered := injectSettingsIntoHTML(baseHTML, settingsJSON, c.Request.URL.Path)
	stored := cache.SetForKeyIfGeneration(cacheKey, rendered, settingsJSON, cacheGeneration)
...
func (s *FrontendServer) frontendDocument(requestPath string) ([]byte, *HTMLCache) {
	if isAdminFrontendPath(requestPath) {
		return s.adminBaseHTML, s.adminCache
	}
	return s.baseHTML, s.cache
}
...
func (s *FrontendServer) injectSettingsForPath(settingsJSON []byte, requestPath string) []byte {
	baseHTML, _ := s.frontendDocument(requestPath)
	return injectSettingsIntoHTML(baseHTML, settingsJSON, requestPath)
}

func injectSettingsIntoHTML(baseHTML, settingsJSON []byte, requestPath string) []byte {
...
var (
	...
	robotsMetaNamePattern = regexp.MustCompile(`(?i)\bname\s*=\s*["']robots["']`)
	noIndexContentPattern = regexp.MustCompile(`(?i)\bcontent\s*=\s*["'][^"']*\bnoindex\b[^"']*["']`)
)

func hasNoIndexMetadata(html []byte) bool {
	for _, tag := range metaTagPattern.FindAll(html, -1) {
		if robotsMetaNamePattern.Match(tag) && noIndexContentPattern.Match(tag) {
			return true
		}
	}
	return false
...
func ServeEmbeddedFrontend() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if isFrontendEntryRequest(path, cleanPath) {
...
func serveIndexHTML(c *gin.Context, fsys fs.FS) {
	serveEmbeddedHTMLFile(c, fsys, frontendIndexFilePath(c.Request.URL.Path))
}
...
func HasEmbeddedFrontend() bool {
	for _, path := range []string{userFrontendIndexFilePath, adminFrontendIndexFilePath} {
		if _, err := frontendFS.ReadFile("dist/" + path); err != nil {
			return false
		}
	}
	return true
