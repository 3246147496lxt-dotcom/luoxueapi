...
func (s *FrontendServer) injectSettingsForPath(settingsJSON []byte, requestPath string) []byte {
	...
	if htmlRouteCacheKey(requestPath) == modelCatalogHTMLCacheKey {
		result = injectModelCatalogMetadata(result, settingsJSON)
	}

	return result
}

const modelCatalogHTMLCacheKey = "models.html"

func htmlRouteCacheKey(requestPath string) string {
	if strings.TrimSpace(requestPath) == "/models.html" {
...
// document head before Vue mounts. The canonical URL is deliberately relative:
// it resolves against the deployed origin without trusting a request Host.
func injectModelCatalogMetadata(html, settingsJSON []byte) []byte {
	...
	metadata := []byte(
		`<meta name="description" content="` + htmlpkg.EscapeString(description) + `" />` +
			`<link rel="canonical" href="/models.html" />` +
			...
			`<meta property="og:url" content="/models.html" />`,
	)
	return bytes.Replace(result, []byte("</head>"), append(metadata, []byte("</head>")...), 1)
}
