...
type APIKeyQuery struct {
	...
	withManagedDevice *DesktopDeviceQuery
...
// QueryManagedDevice chains the current query on the "managed_device" edge.
func (_q *APIKeyQuery) QueryManagedDevice() *DesktopDeviceQuery {
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withManagedDevice: _q.withManagedDevice.Clone(),
...
// WithManagedDevice tells the query-builder to eager-load the nodes that are connected to
// the "managed_device" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithManagedDevice(opts ...func(*DesktopDeviceQuery)) *APIKeyQuery {
	query := (&DesktopDeviceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withManagedDevice = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withManagedDevice; query != nil {
		if err := _q.loadManagedDevice(ctx, query, nodes, nil,
			func(n *APIKey, e *DesktopDevice) { n.Edges.ManagedDevice = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadManagedDevice(ctx context.Context, query *DesktopDeviceQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *DesktopDevice)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*APIKey)
	for i := range nodes {
		if nodes[i].ManagedDeviceID == nil {
			continue
		}
		fk := *nodes[i].ManagedDeviceID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "managed_device_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *APIKeyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withManagedDevice != nil {
			_spec.Node.AddColumnOnce(apikey.FieldManagedDeviceID)
		}
	}
