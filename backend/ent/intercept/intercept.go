...
// The ModelCatalogModelFunc type is an adapter to allow the use of ordinary function as a Querier.
type ModelCatalogModelFunc func(context.Context, *ent.ModelCatalogModelQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ModelCatalogModelFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ModelCatalogModelQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ModelCatalogModelQuery", q)
}

// The TraverseModelCatalogModel type is an adapter to allow the use of ordinary function as Traverser.
type TraverseModelCatalogModel func(context.Context, *ent.ModelCatalogModelQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseModelCatalogModel) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseModelCatalogModel) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ModelCatalogModelQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ModelCatalogModelQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.ModelCatalogModelQuery:
		return &query[*ent.ModelCatalogModelQuery, predicate.ModelCatalogModel, modelcatalogmodel.OrderOption]{typ: ent.TypeModelCatalogModel, tq: q}, nil
