...
金额使用十进制定点字符串，避免 JavaScript 浮点误差。`used_percent` 仅用于显示，不能参与账务计算。
`wallet.unit` 当前仍返回兼容旧客户端的 `snow_credit` 标识；客户端用户界面统一将该
单位显示为“积分”（英文为 “points”），不得再渲染旧货币符号。
...
| `503` | 无法生成快照且服务端没有可安全返回的旧快照，或服务端处于绕过额度拦截与累计的 `simple` 模式。 |
