...
Commands:
  ...
  up [--recreate]       Create and start the complete LuoxueAPI stack
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another LuoxueAPI Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
create_app_container() {
    info "Creating LuoxueAPI container..."
    container create \
        --name "${APP_CONTAINER}" \
        --label "${STACK_LABEL_KEY}=${STACK_LABEL_VALUE}" \
        --network "${NETWORK_NAME}" \
...
start_app() {
    ...
    if ! wait_for_probe "LuoxueAPI" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "LuoxueAPI did not become ready."
    fi
    if ! wait_for_probe "LuoxueAPI host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "LuoxueAPI is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "LuoxueAPI stack stopped; persistent volumes were preserved."
}
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the LuoxueAPI stack and all persistent data? [y/N] '
    else
        printf 'Delete the LuoxueAPI containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "LuoxueAPI stack and persistent data deleted."
    else
        info "LuoxueAPI stack deleted; persistent volumes were preserved."
    fi
}
