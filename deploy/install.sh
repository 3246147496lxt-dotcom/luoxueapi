#!/bin/bash
#
# LuoxueAPI Installation Script
# 落雪API安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/3246147496lxt-dotcom/luoxueapi/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="3246147496lxt-dotcom/luoxueapi"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="落雪API安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动落雪API服务："
    ...
    ["not_installed"]="落雪API尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除落雪API。"
    ...
    ["server_config_desc"]="配置落雪API服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="LuoxueAPI Installation Script"
    ...
    # Completion
    ["install_complete"]="LuoxueAPI installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start LuoxueAPI service:"
    ...
    ["not_installed"]="LuoxueAPI is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove LuoxueAPI from your system."
    ...
    ["uninstall_complete"]="LuoxueAPI has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install LuoxueAPI"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove LuoxueAPI"
    ...
    ["server_config_desc"]="Configure LuoxueAPI server listen address"
...
[Unit]
Description=LuoxueAPI - AI API Gateway Platform
Documentation=https://github.com/3246147496lxt-dotcom/luoxueapi
...
# Upgrade function
upgrade() {
    # Check if LuoxueAPI is installed
    if [ ! -f "$INSTALL_DIR/sub2api" ]; then
...
# Requires: LuoxueAPI must already be installed
install_version() {
